...
| Q4 | Crate names. **Taken** on crates.io: `stratum` and `stratum-core` (Stratum V2, a Bitcoin mining protocol, actively published), plus many `nexus-stratum-*` UI crates. **Free** (re-verified 2026-09-17): every name the workspace uses: `libstratum`, `libstratum-model`, `libstratum-core`, `libstratum-arch`, `libstratum-demangle`, `libstratum-lang-cpp`, `libstratum-format-elf`, `libstratum-format-macho`, `libstratum-format-pe`, `libstratum-debug-dwarf`, `libstratum-debug-pdb`, and the planned `libstratum-cli`. Homebrew formula `stratum`: free. Note that `stratum-core` (taken, unrelated) and `libstratum-core` (ours, free) differ only by the prefix. Nothing is reserved until we publish; re-check before M5 | — | **Resolved:** `libstratum-*` prefix for every crate, CLI binary `stratum` |
...
| Q16 | Best-effort layout numbers for types with virtual bases? Options: (a) report holes only up to the first byte a hidden member could occupy; (b) reconstruct hidden members from the ABI (Itanium vptr/virtual-base placement, MSVC vbptr) and mark them `Evidence::Computed`; (c) keep reporting members only | M3–M4 | (b) behind explicit evidence, validated against `pahole`/`llvm-pdbutil` on the inheritance fixtures |
